...
// Ensure DoNotEscape is not escaped
func TestEncode(t *testing.T) {
	if SqlEncodeMap[DoNotEscape] != DoNotEscape {
		t.Errorf("Encode fail: %v", SqlEncodeMap[DoNotEscape])
	}
	if SqlDecodeMap[DoNotEscape] != DoNotEscape {
		t.Errorf("Decode fail: %v", SqlDecodeMap[DoNotEscape])
	}
}
