...
func (e *Error) ToSQLError() *mysql2.SQLError {
	code := e.getMySQLErrorCode()
	return mysql2.NewErrf(code, "%s", e.getMsg())
}
...
func (e *Error) getMySQLErrorCode() uint16 {
	...
	if !ok {
		zap.S().Warnf("Unknown error class: %v", e.class)
		return defaultMySQLErrorCode
	}
	code, ok := codeMap[e.code]
	if !ok {
		zap.S().Debugf("Unknown error class: %v code: %v", e.class, e.code)
		return defaultMySQLErrorCode
	}
	return code
}
...
func init() {
	...
	defaultMySQLErrorCode = mysql.ErrCodeUnknown
}
...
func MustNil(err error, closeFuns ...func()) {
	if err != nil {
		...
		zap.S().Fatalf(errors.ErrorStack(err))
	}
}
...
func Call(fn func() error) {
	err := fn()
	if err != nil {
		zap.S().Error(errors.ErrorStack(err))
	}
}
