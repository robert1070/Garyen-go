...
type Scanner struct {
	...
	// For scanning such kind of comment: /*! baseMySQL-specific code */ or /*+ optimizer hint */
...
func (s *Scanner) scan() (tok int, pos Pos, lit string) {
	if s.specialComment != nil {
		...
		// for scanning such kind of comment: /*! baseMySQL-specific code */
...
func startWithSlash(s *Scanner) (tok int, pos Pos, lit string) {
	...
	if ch0 == '*' {
		...
		// Convert "/*!VersionNumber baseMySQL-specific-code */" to "baseMySQL-specific-code".
		if strings.HasPrefix(comment, "/*!") {
