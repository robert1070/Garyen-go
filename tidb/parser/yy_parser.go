...
const (
	codeErrParse  = terror.ErrCode(mysql.ErrCodeParse)
	codeErrSyntax = terror.ErrCode(mysql.ErrCodeSyntax)
)

var (
	// ErrSyntax returns for sql syntax error.
	ErrSyntax = terror.ClassParser.New(codeErrSyntax, mysql.ErrName[mysql.ErrCodeSyntax])
	// ErrParse returns for sql parse error.
	ErrParse = terror.ClassParser.New(codeErrParse, mysql.ErrName[mysql.ErrCodeParse])
...
func init() {
	parserMySQLErrCodes := map[terror.ErrCode]uint16{
		codeErrSyntax: mysql.ErrCodeSyntax,
		codeErrParse:  mysql.ErrCodeParse,
	}
	terror.ErrClassToMySQLCodes[terror.ClassParser] = parserMySQLErrCodes
}

// TrimComment trim comment for special comment code of baseMySQL.
...
func New() *Parser {
	if ast.NewValueExpr == nil ||
		...
		panic("no parser driver (forgotten import?) https://Garyen-go/tidb/parser/issues/43")
	}
