...
import (
	...
	"Garyen-go/tidb/parser/stmtctx"
	"Garyen-go/tidb/parser/terror"
	tidbMath "Garyen-go/tidb/util/math"
...
var (
	_, _ = parseTwoNumeric("")
	_, _ = abbreviatedWeekday(nil, "", nil)
	_, _ = dayOfMonthWithSuffix(nil, "", nil)
)
...
// Duration is the type for baseMySQL TIME type.
...
// Fsp is in range [0, 6].
// baseMySQL supports many valid datetime format, but still has some limitation.
...
func checkDateRange(t MysqlTime) error {
	// Oddly enough, baseMySQL document says date range should larger than '1000-01-01',
...
// the converted value not exceed the range of baseMySQL's TIME type.
...
// baseMySQL permits any punctuation delimiter in the expr format.
...
var dateFormatParserTable = map[string]dateFormatParser{
	...
	// Deprecated since baseMySQL 5.7.5
