...
func (bj BinaryJSON) Unquote() (string, error) {
	switch bj.TypeCode {
	case TypeCodeString:
		tmp := string(util.String(bj.GetString()))
	...
	for i := 0; i < len(s); i++ {
		if s[i] == '\\' {
			...
			case 'u':
				...
				char, size, err := decodeEscapedUnicode(util.Slice(s[i+1 : i+5]))
				if err != nil {
					return "", errors.Trace(err)
				}
...
// And baseMySQL use 4 bytes to represent the unicode which must be in [0, 65536).
...
func (bj BinaryJSON) extractTo(buf []BinaryJSON, pathExpr PathExpression) []BinaryJSON {
	...
	if currentLeg.typ == pathLegIndex {
		...
		if currentLeg.dotKey == "*" {
			...
			child, ok := bj.objectSearchKey(util.Slice(currentLeg.dotKey))
			if ok {
...
func (bm *binaryModifier) doInsert(path PathExpression, newBj BinaryJSON) {
	...
	bm.modifyPtr = &parentBj.Value[0]
	elemCount := parentBj.GetElemCount()
	insertKey := util.Slice(lastLeg.dotKey)
...
func (bm *binaryModifier) doRemove(path PathExpression) {
	...
	bm.modifyPtr = &parentBj.Value[0]
	elemCount := parentBj.GetElemCount()
	removeKey := util.Slice(lastLeg.dotKey)
	keys := make([][]byte, 0, elemCount+1)
	elems := make([]BinaryJSON, 0, elemCount+1)
	for i := 0; i < elemCount; i++ {
...
// rules referenced by baseMySQL JSON_DEPTH function
