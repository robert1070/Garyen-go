...
(
  ...
  default character set = utf8mb4 comment '实例表(实例录入)';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='资源申请表';
...
alter table tb_core_sql_order
    add `perm_detail` json COMMENT '权限申请详情' after `remark`,
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='工单详情';
...
(
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='工单流转记录';
...
(
    ...
    `env`            tinyint(4) unsigned not null comment '环境类型: 1-生产、2-预发、3-测试、4-开发',
    `perm_type`      varchar(16)         not null comment '权限类型，' 1, 2, 3
    '(1-数据变更，2-结构变更，3-数据导出)',
    `data_source_id` int                 not null COMMENT '实例id, 来源于tb_walnut_data_source.id',
  ...
  DEFAULT CHARSET = utf8mb4 COMMENT ='用户权限管理表';